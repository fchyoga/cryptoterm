...
type Config struct {
	...
	// AI Signal Copilot settings
	AIProvider     string `json:"ai_provider"`      // "gemini", "openrouter", "deepseek", "groq", "openai", "anthropic", "ollama", "custom"
	AIAPIKey       string `json:"ai_api_key"`       // API key for the selected AI provider
	AIBaseURL      string `json:"ai_base_url"`      // Custom base URL (optional, for 9router, local, or proxy)
	AIModel        string `json:"ai_model"`         // Custom model name (e.g. "gemini-2.0-flash", "deepseek-chat")
	AITradingStyle string `json:"ai_trading_style"` // "scalper", "daytrader", "degen"
}
...
// Kline represents a single candlestick interval.
type Kline struct {
	OpenTime  time.Time
	Open      float64
	High      float64
...
// AISignal holds a structured trading recommendation generated by an LLM.
type AISignal struct {
	Symbol           string    `json:"symbol"`
	DisplaySymbol    string    `json:"display_symbol"`
	Action           string    `json:"action"`            // STRONG_BUY, BUY, NEUTRAL, SELL, STRONG_SELL
	Confidence       float64   `json:"confidence"`        // 1.0 to 10.0
	RiskLevel        string    `json:"risk_level"`        // LOW, MEDIUM, HIGH
	EntryZone        string    `json:"entry_zone"`        // e.g. "$105.00 - $106.20"
	TakeProfit1      float64   `json:"take_profit_1"`     // Target 1
	TakeProfit2      float64   `json:"take_profit_2"`     // Target 2
	StopLoss         float64   `json:"stop_loss"`         // Invalidation level
	RiskReward       string    `json:"risk_reward"`       // e.g. "1 : 2.5"
	Reasoning        []string  `json:"reasoning"`         // Key bullet points
	Invalidation     string    `json:"invalidation"`      // When to immediately exit
	TechnicalSummary string    `json:"technical_summary"` // Summary of RSI, EMA, volume
